...
describe("VeracityClient", () => {
  it("sends bearer auth to analyze audio", async () => {
    ...
    const result = await client.analyzeAudio({ audio_url: "https://example.com/clip.mp3", privacy_mode: true, context: { format: "other", intended_use: "other" } });

    expect(result).toMatchObject({ analysis_id: "aud_1" });
    expect(fetchMock).toHaveBeenCalledWith("https://api.example.test/v1/analyze-audio", expect.objectContaining({
      method: "POST",
