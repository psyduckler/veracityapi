...
function extensionConnectScript(redirectUri: string, state: string): string {
  ...
  return `(function(){var key='veracity_extension_redirect';var expected=${expectedState};function go(url){if(url){window.location.href=url;}}window.addEventListener('storage',function(event){if(event.key!==key||!event.newValue)return;try{var data=JSON.parse(event.newValue);if(data&&data.state===expected&&data.redirect_url)go(data.redirect_url);}catch(_){}});var form=document.querySelector('[data-extension-connect-form]');if(!form)return;var status=document.querySelector('[data-extension-status]');var button=form.querySelector('button');form.addEventListener('submit',async function(event){event.preventDefault();try{if(button){button.disabled=true;button.textContent='Connecting…';}if(status)status.textContent='Finishing connection…';var response=await fetch(form.action,{method:'POST',body:new URLSearchParams(new FormData(form)),headers:{'accept':'application/json','content-type':'application/x-www-form-urlencoded'},credentials:'same-origin'});if(!response.ok)throw new Error('Connection failed. Refresh and try again.');var data=await response.json();if(!data.redirect_url)throw new Error('Connection response was missing redirect URL.');localStorage.setItem(key,JSON.stringify({state:data.state,redirect_url:data.redirect_url,at:Date.now()}));go(data.redirect_url);}catch(error){if(button){button.disabled=false;button.textContent='Connect extension';}if(status)status.textContent=error&&error.message?error.message:'Connection failed. Refresh and try again.';}});})();`;
}

function css(): string { return `:root{color-scheme:light;--bg:#f6f1df;--panel:#fffdf4;--text:#15120d;--muted:#655d52;--line:#2b2418;--accent:#ff4fd8;--cyan:#00c8ff}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 12% -8%,rgba(255,79,216,.15),transparent 24rem),linear-gradient(180deg,#fff8cf 0,#f5efe0 42%,#e2ddcf 100%);color:var(--text);font-family:Inter,system-ui,sans-serif}.wrap{min-height:100vh;display:grid;place-items:center;padding:24px}.card{max-width:560px;border:2px solid var(--line);background:var(--panel);box-shadow:6px 6px 0 var(--line);border-radius:8px;padding:28px}h1{margin:0 0 12px;font-size:28px;letter-spacing:-.04em}p{color:var(--muted);line-height:1.6}.btn{border:2px solid var(--line);border-radius:4px;padding:11px 14px;text-decoration:none;display:inline-flex;background:linear-gradient(180deg,#fffef6,#d5cdbc);box-shadow:3px 3px 0 var(--line);color:var(--text);font-weight:800}.btn.primary{background:linear-gradient(135deg,var(--accent),var(--cyan));color:#120916}.error{color:#9b1c1c}.ok{color:#087f4f}code{background:#fff7d8;border:1px solid rgba(43,36,24,.18);padding:2px 5px;border-radius:4px}`; }
